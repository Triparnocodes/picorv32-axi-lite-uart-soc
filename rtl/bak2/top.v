...
module top (
    ...
    // UART – Slave 2
    // =========================================================================
    // FIX: "uart_tx has no driver"
    // -----------------------------------------------------------------------
    // Yosys opt_clean can confuse the top-level output PORT named `uart_tx`
    // with the sub-module TYPE named `uart_tx` instantiated inside uart_axi.
    // Routing through a uniquely named (* keep *) wire gives Yosys an
    // unambiguous single driver for the top-level output in every opt pass.
    // -----------------------------------------------------------------------
    (* keep *) wire uart_tx_w;
    assign uart_tx = uart_tx_w;          // top output is explicitly driven
    ...
    uart_axi #(.CLK_FREQ(50_000_000), .BAUD_RATE(115_200)) u_uart (
        ...
        .uart_tx(uart_tx_w),      // ← drive (* keep *) wire, not port directly
        .uart_rx(uart_rx)
    );
