...
module uart_axi #(
    ...
    // FIX: replace (* keep *) assign with buf primitive
    // -----------------------------------------------------------------------
    // `assign uart_tx = tx_serial_out` can be silently dropped by Yosys
    // opt_clean when the module type `uart_tx` is also instantiated here,
    // creating a naming collision.  A Verilog `buf` primitive is always
    // mapped to a real buffer cell in the netlist and is NEVER eliminated
    // by any Yosys optimization pass, making this connection synthesis-safe.
    // -----------------------------------------------------------------------
    wire tx_serial_out;
    buf u_uart_tx_buf (uart_tx, tx_serial_out);  // permanent buffer cell
    ...
    uart_tx #(
        ...
        .valid (tx_valid),
        .ready (tx_ready),
        .tx    (tx_serial_out)   // → buf → uart_tx port
    );
